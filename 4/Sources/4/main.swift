...
struct Passport {
    ...
    var hasAllFields: Bool {
        return [hgt, eyr, byr, iyr, pid, ecl, hcl].filter { $0 == nil }.count == 0
    }
    ...
    var fieldsAreValid: Bool {
        ...
        func isValidYear(_ year: String?, min: Int, max: Int) -> Bool {
            guard let year = year, let yearNum = Int(year) else { return false }
            return year.matches(regex: "\\d{4}") && yearNum >= min && yearNum <= max
        }
        
        func isValidHeight(_ height: String) -> Bool {
            guard height.matches(regex: "\\d+(cm|in)") else { return false }
            
            if height.contains("cm") {
                let height = Int(height.replacingOccurrences(of: "cm", with: ""))!
                return height >= 150 && height <= 193
            } else {
                let height = Int(height.replacingOccurrences(of: "in", with: ""))!
                return height >= 59 && height <= 76
            }
        }
        
        
        return isValidYear(byr, min: 1920, max: 2002)
            && isValidYear(eyr, min: 2020, max: 2030)
            && isValidYear(iyr, min: 2010, max: 2020)
    ...
    init(string: String) {
        ...
        parts.forEach {
            ...
            case "ecl": ecl = value
            case "hcl": hcl = value
            default: fatalError("Unknown key \(key)")
            }
        }
    }
}

let passports = input.map { Passport(string: $0) }

func part1() {
    let validCount = passports.filter { $0.hasAllFields }.count
    print("\(validCount) passports are valid")
}

func part2() {
    let validCount = passports.filter { $0.isValid }.count
    print("\(validCount) passports are valid")
}

passports.filter { $0.isValid}.sorted(by: { $0.pid! < $1.pid! }).forEach {
    print($0.pid!)
}
