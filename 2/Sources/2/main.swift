...
struct Policy {
    ...
    init(_ string: String) {
        ...
        let nums = parts[0].components(separatedBy: "-")
        minCount = Int(nums[0])!
        maxCount = Int(nums[1])!
        letter = parts[1]
    }
...
let parts: [(Policy, String)] = input.map {
    let parts = ($0 + " ").components(separatedBy: ": ")
    return (Policy(parts[0]), parts[1])
}

func part1() {
    func isValid(_ password: String, for policy: Policy) -> Bool {
        let occurrences = password.components(separatedBy: policy.letter).count - 1
        return (policy.minCount <= occurrences) && (occurrences <= policy.maxCount)
    }

    print(parts.filter { isValid($0.1, for: $0.0) }.count)
}

func part2() {
    func chars(for policy: Policy, in string: String) -> [String] {
        let char1 = string[policy.minCount - 1]
        let char2 = string[policy.maxCount - 1]
        return [char1, char2]
    }
    func isValid(_ password: String, for policy: Policy) -> Bool {
        let char = chars(for: policy, in: password)
        let count = char.filter { $0.count == 1 }.count
        guard count == 2 else { return false }
        return (char.filter { $0 == policy.letter }.count == 1)
    }
    
    print(parts.filter {
        let valid = isValid($0.1, for: $0.0)
        let char = chars(for: $0.0, in: $0.1)
        print($0.1, $0.0.minCount, char[0], $0.0.maxCount, char[1], $0.0.letter, valid)
        return valid
    }.count)
}
